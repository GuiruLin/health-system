...
export default async function handler(req, res) {
  ...
  if (secret) {
    ...
    if (auth !== `Bearer ${secret}` && key !== secret) return res.status(403).json({ error: "Forbidden" });
  }
  ...
  try {
    ...
    const last14 = dailyE.slice(-14);
    const sleep7 = last14.slice(-7).map(d => ({ date: d.date, hrs: rnd1(Number(d.sleepTotal)), deep: rnd1(Number(d.deep)), rem: rnd1(Number(d.rem)) }));
    const sleepAvg = rnd1(mean(last14.slice(-7), d => Number(d.sleepTotal)));
    const deepAvg = rnd1(mean(last14.slice(-7), d => Number(d.deep)));
    const hrvDays = last14.filter(d => d.hrv);
    const hrvBase = hrvDays.length ? Math.round(mean(hrvDays, d => Number(d.hrv))) : null;
    const hrvLatest = hrvDays.length ? Math.round(Number(hrvDays[hrvDays.length - 1].hrv)) : null;
    const rhrDays = last14.filter(d => d.rhr);
    const rhrBase = rhrDays.length ? Math.round(mean(rhrDays, d => Number(d.rhr))) : null;
    ...
    const diet7 = dates7.map(dt => {
      ...
      return { date: dt, protein: dm.reduce((s, m) => s + (Number(m.protein) || 0), 0), fibre: dm.reduce((s, m) => s + (Number(m.fibre) || 0), 0) };
    }).reverse();
    ...
    const summary = {
      ...
      diet: { protein_avg_g: proteinAvg, protein_target_g: proteinTarget, protein_target_hit_days_of7: proteinHitDays, fibre_avg_g: fibreAvg, fibre_target_g: fibreTarget, last7: diet7 },
    ...
    const system =
      ...
      "一周的大致构成(智慧默认,按她状态灵活增减,不是死规则):约2次大重量力量(下肢+上肢,两天分开);1次间歇或Tempo(二选一,直接写清是「间歇」还是「Tempo」,绝不要用『质量课』这个说法;都要冲进Z4即166+,间歇要摸到180+);1次逐周推进的长距离(约+2km往~30km峰值、赛前约3周后减量);1-2次Z2轻松跑(心率130-145);约1次普拉提/Barre;至少1天完全休息。极化80/20为总原则。" +
      "把这些聪明地摆进7天:最硬的课(间歇或Tempo/大重量/长距离)放她最新鲜的日子,完全休息与轻松日放她恢复最差的日子;练腿的硬日彼此错开(长距离/间歇/重腿之间夹轻松或上肢或休息);卵泡期可多推力量与强度,黄体期或月经期偏向恢复与补给;她的长距离通常放周末(有整段时间),她也重视一天真正的休息。" +
    ...
    const aiRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: MODEL, max_tokens: 3500, system, messages: [{ role: "user", content: "我过去一周的全部数据:\n" + JSON.stringify(summary, null, 2) }] }),
    });
